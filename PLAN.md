...
Status (2026-07-23): **TASK_02, TASK_15, TASK_11, TASK_18, TASK_19, TASK_21 complete.** Repo check green. **Next: TASK_22** (role-override repositioning, all bands) then **TASK_07 recalibration** (TASK_19 moved the gap curve + fatigue; TASK_21 moved live positions) — plus TASK_16/17 if desired before the lock. TASK_12 (mundane fouls) PARKED. TASK_18 shipped attacker-side loser-fouls; TASK_19 shipped ball-side cover + unified movement-distance fatigue; TASK_21 shipped role-aware canonical width.

## Just shipped — TASK_21 (role-aware canonical width)

Width in `canonicalGeometry` ([lineup.ts](packages/match/src/lineup/lineup.ts)) is now role-aware: even spacing scaled to a span set by the row's **edge role** — `WIDE_EDGE_LATERAL 0.6` (flank edge: LB/RB/LM/RM/LW/RW) / `CENTRAL_EDGE_LATERAL 0.5` (central edge: CB/DM/CM/AM/ST); `isWideRole(role)` + both constants exported. `lateral = edge * even`. Effect: 4-back x 0.2/0.4/0.6/0.8, 3-back CB trio x 0.25/0.5/0.75, front twos/double pivots tucked off the touchline. `deriveRolesForShape` unaffected (labels by index/count, not magnitude). `ROLE_CANONICAL_LATERAL` left ±1 → revisited in TASK_22. Two calibration gates eased by the position move (TASK_07 re-locks): `simulate.test.ts` strong-side wins 45→40/60; `scale-calibration.test.ts` out-shoot ratio 1.7→1.6. `WIDE_EDGE_LATERAL`/`CENTRAL_EDGE_LATERAL` are starting values — tune in TASK_07. See [TASK_21.md](TASK_21.md).
...
| 22 | `TASK_22.md` | **Role-override repositioning (all bands)** — a role override only relabels + sets the flank; it doesn't move the anchor, so CB→LB isn't a visible lateral shift. Make an override nudge the player to the role's width (reusing TASK_21's `isWideRole` + edge constants), consistently across defence/midfield/attack; revisit `ROLE_CANONICAL_LATERAL`. Surfaced during TASK_21 planning. | After TASK_21; before TASK_07 |
...
5. ✅ `TASK_19` — defensive cover + lateral fatigue — DONE
   - ✅ `TASK_21` — role-aware canonical width (nothing on the touchline) — DONE 2026-07-23
   - `TASK_22` — role-override repositioning (all bands); surfaced by TASK_21, do after it, before TASK_07 — **do next**
