...
Status (2026-07-23): **TASK_02, TASK_15, TASK_11, TASK_18, TASK_19 complete.** Repo check green. **Next: TASK_07 recalibration** (TASK_19 moved the gap curve + fatigue; also fold in TASK_21 role-aware back-line width before locking) — plus TASK_16/17 if desired before the lock. TASK_12 (mundane fouls) PARKED — re-evaluate only if 18+19 leave the foul distribution unrealistic. TASK_18 shipped attacker-side loser-fouls; TASK_19 shipped ball-side cover + unified movement-distance fatigue.
...
| 21 | `TASK_21.md` | **Role-aware back-line width** — `canonicalGeometry` spaces a back line purely by slot order, so a back-three's CBs spread touchline-to-touchline (x=0/0.5/1.0) instead of tucking; `LB-CB-RB` and `CB-CB-CB` get identical geometry. Make lateral width role-aware (CBs tuck central, fullbacks/wing-backs wide). Surfaced during TASK_19. Moves live positions → **TASK_07 recalibration must follow**. | After/with TASK_19; before TASK_07 |
...
5. ✅ `TASK_19` — defensive cover + lateral fatigue — DONE
   - `TASK_21` — role-aware back-line width (CBs tuck, wing-backs stretch); surfaced by TASK_19, do before TASK_07 — **do next (or straight to TASK_07)**
