...
Status (2026-07-23): **TASK_02, TASK_15, TASK_11, TASK_18, TASK_19, TASK_21, TASK_22 complete.** Repo check green. **Next: TASK_16 → TASK_17 → TASK_07 recalibration.** TASK_12 (mundane fouls) PARKED. TASK_23 (formation editor UI) is a placeholder pending UX discussion.

## Just shipped — TASK_21 + TASK_22 (role-aware band positioning)

**TASK_21** set `WIDE_EDGE_LATERAL 0.6` / `CENTRAL_EDGE_LATERAL 0.5` and made the canonical geometry check the edge role. **TASK_22** completed the model: new `positionsFromBands(bands: FormationPosition[][])` in [lineup.ts](packages/match/src/lineup/lineup.ts) — pure, formation-agnostic, no slot indices. Left/right edges detected independently from the outermost roles in a band; all slots evenly spaced between those edges. `canonicalGeometry` now delegates to `positionsFromBands`. Override pattern: caller patches the band role array and calls `positionsFromBands` — geometry layer has no notion of overrides. `seedShapesFromFormation` stays simple (no overrides param). `lateralsForBand` is internal. See [TASK_21.md](TASK_21.md) and [TASK_22.md](TASK_22.md).
...
| 23 | `TASK_23.md` | **Formation editor: wide/central slot configuration** — UI to let the user toggle slots wide/central per band; calls `positionsFromBands` with the patched bands. Placeholder — pending UX discussion with user. | After TASK_22 |
