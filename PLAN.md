...
Status (2026-07-23): **TASK_02, TASK_15, TASK_11, TASK_18, TASK_19, TASK_21, TASK_22, TASK_23 complete.** Repo check green. **Next: TASK_16 → TASK_17 → TASK_07 recalibration.** TASK_12 (mundane fouls) PARKED.

## Just shipped — TASK_21 + TASK_22 + TASK_23 (role-aware band positioning + formation editor UI)

**TASK_21** set `WIDE_EDGE_LATERAL 0.75` / `CENTRAL_EDGE_LATERAL 0.42` and made the canonical geometry check the edge role. **TASK_22** completed the model: new `positionsFromBands(bands: FormationPosition[][])` in [lineup.ts](packages/match/src/lineup/lineup.ts) — pure, formation-agnostic. Left/right edges detected independently from outermost roles; all slots evenly spaced. `canonicalGeometry` delegates to it. **TASK_23** wired positionsFromBands into the live UI: `recomputeShapeLaterals` in ClubManager calls `deriveRolesForShape` then `positionsFromBands` so lateral positions always match display labels; `TacticsPitch` uses absolute positioning (band rows `position:relative`, slots `position:absolute` with `left:calc(((lateral+1)/2)*100% - slotW/2px)`); drag-to-reposition also snaps via the same recompute; `roleForBandSlot` DEF band now lateral-aware (LB/RB when `Math.abs(lateral) > CENTRAL_EDGE_LATERAL`); `SECONDARY_POSITIONS` LB/RB now include CB/LM and CB/RM so players can reset from wide.
...
| 19 | `TASK_19.md` | ✅ **Defensive cover / ball-side lateral shift (+ lateral fatigue)** — DONE 2026-07-23. `applyBallSideShift` (proximity-decayed, depth-weighted, pure) in the defending motion chain; unified movement-distance fatigue (`perMinuteDrain(distanceMoved)` × `staminaTravelFactor`, via pure `travelled()` + one-min-lag), replacing the static `shapeDeltaDrain`/`derivedRoles`. Spun off TASK_21. **TASK_07 must still follow** to re-lock gap/fatigue/foul gates. | Done |
...
| 21 | `TASK_21.md` | ✅ **Role-aware canonical width (nothing on the touchline)** — DONE 2026-07-23. `canonicalGeometry` now scales even spacing to a span set by the edge role (`WIDE_EDGE_LATERAL 0.6` flank / `CENTRAL_EDGE_LATERAL 0.5` central; `isWideRole` exported). Back-three tucks central, front twos/pivots off the touchline; `deriveRolesForShape` unaffected. `ROLE_CANONICAL_LATERAL` left for TASK_22. Two gates eased (TASK_07 re-locks). | Done |
| 22 | `TASK_22.md` | ✅ **Role-aware band positioning (wide/central distributor)** — DONE 2026-07-23. `positionsFromBands(bands)`: pure, formation-agnostic, no slot indices. Left/right edges set independently from outermost roles; all slots evenly spaced. Override pattern: caller patches band role arrays before calling `positionsFromBands`. `canonicalGeometry` delegates to it. | Done |
| 23 | `TASK_23.md` | ✅ **Formation editor: role-aware positioning wired to UI** — DONE 2026-07-23. `recomputeShapeLaterals` in ClubManager, absolute-position TacticsPitch, drag-snap, lateral-aware DEF labelling, LB/RB secondaries. | Done |
...
5. ✅ `TASK_19` — defensive cover + lateral fatigue — DONE
   - ✅ `TASK_21` — role-aware canonical width — DONE 2026-07-23
   - ✅ `TASK_22` — role-aware band positioning (positionsFromBands) — DONE 2026-07-23
   - `TASK_23` — formation editor UI (pending UX discussion)
