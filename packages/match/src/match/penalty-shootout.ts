...
/** Per-kick conversion probability for a side, derived from its on-pitch quality. */
function conversionProbability(players: Player[]): number {
  const prob = BASE_CONVERSION + getTeamOVR(players) * OVR_WEIGHT;
 ...
 */
export function simulateShootout(homePlayers: Player[], awayPlayers: Player[], rng: () => number = Math.random): ShootoutResult {
  const homeProb = conversionProbability(homePlayers);
  const awayProb = conversionProbability(awayPlayers);
